...
最新のリビジョンにロールバックしたい場合はこのステップを省略できます。
...
これによってDaemonSetのリビジョンのリストが返ってきます:
...
* Change causeは作成時にDaemonSetのアノテーション`kubernetes.io/change-cause`からコピーされます。
  実行したコマンドをChange causeのアノテーションに記録するために、`kubectl`内で`--record=true`を指定することができます。
...
### ステップ2: 指定したリビジョンにロールバックする

```shell
# --to-revisionによってステップ1で得たリビジョン番号を指定します。
...
`kubectl rollout undo`は、DaemonSetのテンプレートを、`kubectl edit`や`kubectl apply`のような他のコマンドを通して以前のリビジョンに更新することに相当します。
